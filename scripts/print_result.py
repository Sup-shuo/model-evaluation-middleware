#!/usr/bin/env python3
"""Print a saved evaluation result and optionally render a terminal-like SVG.

The report is a convenient view over files already saved in a run directory.
It is not a signature, tamper-proof record, or replacement for the native JSON.
"""
...
def load_object(path: Path) -> dict[str, Any]:
    try:
        ...
        raise SystemExit(f"required result file is missing: {path}") from exc
...
def environment_lines(
    ...
    device_names = ", ".join(str(item.get("name") or item.get("id")) for item in devices) or "N/A"
